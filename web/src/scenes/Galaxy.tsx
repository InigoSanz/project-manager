...
import { ProjectOrb, type Quality } from "../visuals/ProjectOrb";
...
function OrbNode({
  ...
  return (
    <Float speed={1.2} rotationIntensity={0.15} floatIntensity={0.6} position={position}>
      ...
      >
        <ProjectOrb dna={dna} scale={hovered ? 1.18 : 1} livePulse={live ? 1 : 0} quality={quality} />
        <Html
          center
          distanceFactor={small ? 16 : 12}
          position={[0, -dna.radius * 1.9, 0]}
...
export function Galaxy({ projects, liveActivity }: { projects: Project[]; liveActivity: Record<string, number> }) {
  ...
  const quality: Quality = touch || small ? "lite" : "high";
  return (
    <Canvas
      ...
      <Suspense fallback={null}>
        ...
        {projects.map((p, i) => (
          <OrbNode
            ...
            project={p}
            index={i}
            live={now - (liveActivity[p.id] ?? 0) < 60_000}
            quality={quality}
            small={small}
          />
        ))}
        <EffectComposer>
          <Bloom
            intensity={quality === "lite" ? 1.0 : 1.25}
